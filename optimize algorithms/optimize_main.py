...
from plot_help import plotShapelyPoly, PlotPatchHelper, FigureObjects, OutputFcn
...
def callOPT(Order, Stock):
    ...
    for num, stock in enumerate(Stock):
        if areaStock[num] >= areaOrder:
            ...
            bRangeMatrix = ubMatrix
            x0 = lbMatrix + np.random.rand(1,nVars) * bRangeMatrix
            #opt = minimizeCompass(ObjectiveFcn, x0=x0[0], deltatol=0.1, 
            #                      paired=False,args=[stock,Order],errorcontrol=False,disp=True)
           # opt = minimize(ObjectiveFcn, x0, args=(stock,Order),method='Nelder-Mead',
            #               tol=1e-6,options={'maxiter': 10000,'disp': True})
            opt = minimize(ObjectiveFcn, x0, args=(stock,Order),method='L-BFGS-B',
                           tol=1e-6,options={'maxiter': 10000,'disp': True})
            #opt = minimize(ObjectiveFcn, x0, args=(stock,Order),method='SLSQP',
            #               tol=1e-6,options={'maxiter': 10000,'disp': True})
            if checkPlacement(opt.x, Order, stock) < 0.001:
                return opt.fun, opt.x, num
...
Orders are calculated in sequence order1>order2>order3.
 - callPSO calculates the area of order and stock pieces and sequentally looks for an acceptable solution
     in stock pieces whose area is bigger than order area. The first acceptable answer(no overlap area) is returned.
     If there are no pieces that are big enough , callPSO returns a (None, None, None) to avoid exceptions in our code
...
minimizer_results = pd.DataFrame(columns= col)
...
for k in range(1):
    ...
    total_gbf = 0
    while True:
        gbf, gbp, best_stock_num = callOPT(orderList[i],remainingStock)
    ...
    minimizer_results = pd.concat([minimizer_results, pd.DataFrame(results, columns=col)], sort=True )
        


print(minimizer_results)

#minimizer_results.to_excel("SLSQP.xlsx")
