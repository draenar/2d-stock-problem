...
def callPSO(Order, Stock):
    ...
    for num, stock in enumerate(Stock):
        if areaStock[num] >= areaOrder:
            ...
            pso = DynNeighborPSO(objFun, nVars, LowerBounds= LowerBounds, UpperBounds=UpperBounds, 
                 OutputFcn=outFun, UseParallel=False, MaxStallIterations=20)
            pso.optimize()
            if checkPlacement(pso.GlobalBestPosition, Order, stock)<0.001:
                return pso.GlobalBestFitness, pso.GlobalBestPosition, num
...
'''
#create a list with orders


col = ['number of orders', 'placed properly', 'time', 'total fitness']
pso_results = pd.DataFrame(columns= col)
...
for k in range(1):
    ...
    total_gbf = 0
    while True:
        gbf, gbp, best_stock_num = callPSO(orderList[i],remainingStock)
    ...
    pso_results = pd.concat([pso_results, pd.DataFrame(results, columns=col)], sort=True )



print(pso_results)

#pso_results.to_excel("pso.xlsx")
